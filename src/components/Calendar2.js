...
const Calender2 = ({ getState, startDayss, endDayss }) => {
  const Color = useSelector(s => s.SystemReducer.theme);
  const [startDay, setStartDay] = useState(
    startDayss ? moment(startDayss).format('YYYY-MM-DD') : "",
  );
  const [endDay, setEndtDay] = useState(endDayss ? moment(endDayss).format('YYYY-MM-DD') : '');
