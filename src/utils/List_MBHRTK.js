...
const Item_MBHRTK = () => {
  ...
  const [dataMenuMBHRTK, setDataMenuMBHRRE] = useState([]);
  useEffect(() => {
    let dataMenuMBHRTKs = [];
    dataMenuMBHRs.map(item => {
      if (
        item.p_pk === dataMenuMBHRs.filter(i => i.menu_cd === 'MBHRTK')[0].pk
      ) {
        dataMenuMBHRTKs.push(item);
      }
      setDataMenuMBHRRE(dataMenuMBHRTKs);
    });
