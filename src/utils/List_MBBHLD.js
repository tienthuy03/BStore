...
const Item_MBBHLD = () => {
    ...
    let [dataMenuMBBHLD, setDataMenuMBBHLD] = useState([]);

    useEffect(() => {
        let dataMenuMBBHLDs = [];
        dataMenuMBHRs.map(item => {
            if (
                item.p_pk === dataMenuMBHRs.filter(i => i.menu_cd === 'MBBHLD')[0].pk
            ) {
                dataMenuMBBHLDs.push(item);
            }
            setDataMenuMBBHLD(dataMenuMBBHLDs);
        });
    ...
    const renderItem = ({ item, index }) => {
        return (
            <Block justifyContent={"center"} alignCenter={"center"}  >
                <Button
                    shadow
                    nextScreen={() => navigation.navigate(item.menu_cd)}
                    // height={60}
