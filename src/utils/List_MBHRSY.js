...
const Item_MBHRSY = () => {
  ...
  let [dataMenuMBHRSY, setDataMenuMBHRSY] = useState([]);

  useEffect(() => {
    let dataMenuMBHRSYs = [];
    dataMenuMBHRs.map(item => {
      if (
        item.p_pk === dataMenuMBHRs.filter(i => i.menu_cd === 'MBHRSY')[0].pk
      ) {
        dataMenuMBHRSYs.push(item);
      }
      setDataMenuMBHRSY(dataMenuMBHRSYs);
    });
