...
const Item_MBHRDT = () => {
  ...
  const [dataMenuMBHRDT, setDataMenuMBHRRE] = useState([]);

  useEffect(() => {
    let dataMenuMBHRDTs = [];
    dataMenuMBHRs.map(item => {
      if (
        item.p_pk === dataMenuMBHRs.filter(i => i.menu_cd === 'MBHRDT')[0].pk
      ) {
        dataMenuMBHRDTs.push(item);
      }
      setDataMenuMBHRRE(dataMenuMBHRDTs);
    });
